...
async def main():
    ...
    async with VSSClient('127.0.0.1' , 55556) as client:
        ...
        while True:
        	...
        	# Put the value for each feature
            print('VehicleSpeed = ' , VehicleSpeed)
            response = put_feature_value(THING_ID , 'VehicleSpeed' , VehicleSpeed)
            print(response)

            print('EngineSpeed = ' , EngineSpeed)
            response = put_feature_value(THING_ID , 'EngineSpeed' , EngineSpeed)
            print(response)

            print('ThrottlePosition = ' , ThrottlePosition)
            response = put_feature_value(THING_ID , 'ThrottlePosition' , ThrottlePosition)
            print(response)

            print('coolantTemperature = ' , CoolantTemperature)
            response = put_feature_value(THING_ID , 'CoolantTemperature' , CoolantTemperature)
            print(response)
